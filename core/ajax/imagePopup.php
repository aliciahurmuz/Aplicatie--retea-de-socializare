	...
	include '../init.php';
 	if(isset($_POST['showImage']) && !empty($_POST['showImage'])){
		...
		if ($tweet) {
			...
			echo "<p>Eroare: Nu s-au putut obtine datele tweet-ului. Va rugam sa incercati din nou mai tarziu.</p>";
			exit;
		}
	}
?>
					...
					echo '<ul> 
						'.(($getFromU->loggedIn()) ?   '
								...
								<li>
									...
									<ul> 
									  <li><label class="deleteTweet" data-tweet="'.$tweet->tweetID.'">Șterge Tweet-ul</label></li>
									</ul>
								</li>' : '').'
							' : '
